...
plot_blob_exact <- function(dat=NULL) {
...
plot_blob_scaled <- function(dat=NULL, plot_log=FALSE) {
  head(dat)
  dat$Scld=ifelse(dat[,3] == dat[,4], 0.5, 100*dat[,4]/(dat[,3]+dat[,4]))
  
  max_total=max(dat[,5]) * 1.01
  col_lab=names(dat)[1]
...
blob_plot <- function(dat=NULL, out, w=6.5, h=6, pdf=FALSE, type="exact", plot_log=FALSE) {

  dat=read.table(dat, header=TRUE)
  
  if (type == "exact") {
    print("Plot exact k-mer counts")
    p <- plot_blob_exact(dat)
  } else {
    print("plot scaled k-mer counts")
    w=8
    p <- plot_blob_scaled(dat, plot_log)
  }
  p
...
blob_plot(dat = args$file, out = args$output, w = args$xdim, h = args$ydim, pdf = args$pdf, type=args$type, plot_log = args$plot_log)
