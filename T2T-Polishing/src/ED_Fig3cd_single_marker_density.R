...
dat=read.table("input/ED_Fig3/ED_Fig3de_single_cum_10k.hist", header=FALSE)
head(dat)
...
ggplot(data=dat, aes(x=V1, y=V2)) +
    ...
    scale_y_continuous(labels=fancy_scientific) +
    coord_cartesian(xlim=c(0,x_max), ylim=c(0,y_max))
ggsave("output/ED_Fig3c_single_count_max150.jpg", width=3, height = 2.5)
...
ggplot(data=dat, aes(x=V1, y=V3)) +
    ...
    scale_y_continuous(labels=fancy_scientific) +
    coord_cartesian(xlim=c(0,x_max), ylim=c(0,y_max))
ggsave("output/ED_Fig3d_single_cum.jpg", width=3, height = 2.5)


dat$Bases=10000*dat$V2
y_max=max(dat$Bases)*1.1

ggplot(data=dat, aes(x=V1, y=Bases)) +
    geom_line() +
    theme_bw() +
    xlab("Num. markers in 10kb") +
    ylab("Bases") +
