...
dat=read.table("input/Fig1/scaffolding.txt", header = TRUE)
head(dat)
#tail(dat)
...
# Fig. 1a-d
a = plot_bar_flipped_wi_ylabel(dat$ContigNG50, "Contig NG50 (Mbp)", dark_gray, label_comma = FALSE)
b = plot_bar_flipped(dat$ScaffoldNG50, "Scaffold NG50 (Mbp)", orange, label_comma = FALSE)
...
ggsave(file = "output/pub/Fig1a_d.pdf", width = 170, height = 50, g, units = "mm")


###########################################################################
# Fig. 1h Chromosome Size Comparison between Assembled vs. Karyotype
dat=read.table("input/Fig1/karyotype.txt", header = TRUE)
head(dat)
...
ggplot(dat, aes(x = AssembledChrSize, y = KaryotypeSize)) +
  ...
  scale_y_log10(expand = c(0, 0.05), limits=c(1.5, 210),
                breaks = c(3, 10, 30, 100)) +
  xlab("Assembled Chr. Size (Mbp)") + ylab("Karyotype Size (Mbp)") +
  theme(axis.title = element_text(size=6, face = "bold"),
        axis.text  = element_text(size=6))
#ggsave("output/Fig1_karyotype_loglog.png", width = 1.5, height = 1.5)
ggsave("output/pub/Fig1f_karyotype_loglog.pdf", width = 40, height = 40, units = "mm", device=cairo_pdf)
