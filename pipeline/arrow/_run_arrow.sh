...
qsub -V -q phillippy.q,public.q -j y -cwd -hold_jid split.$prefix -N arrow.$prefix -tc 40 -t 1-$NUM_CONTIGS -pe thread 24 -l mem_free=6g -o logs/arrow.$prefix.\$TASK_ID.log -S /bin/bash _consensus_parallel.sh $prefix
END
qsub -V -q phillippy.q,public.q -j y -cwd -N arrow.$prefix -tc 60 -t 5129-$NUM_CONTIGS -pe thread 24 -o logs/arrow.$prefix.\$TASK_ID.log -S /bin/bash _consensus_parallel.sh $prefix

qsub -V -q phillippy.q,public.q -j y -cwd -hold_jid arrow.$prefix -N merge.$prefix -pe thread 1 -o logs/merge.$prefix.\$TASK_ID.log -S /bin/bash _merge.sh $prefix
